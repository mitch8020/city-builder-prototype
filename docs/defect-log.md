...
| BUG-015 | P2       | A live Metro query for `600 CHURCH ST` rendered six repetitive destinations and exposed the provider taxonomy `StreetAddress` in each result. A follow-up still showed the same destination with and without ZIP `37219`.                                           | Provider candidates were rendered verbatim without a product-level uniqueness or copy boundary.                                        | Address results now collapse punctuation and trailing ZIP/ZIP+4 variants without merging directional streets, translate service types into plain language, and announce searching/result counts. Unit and browser regressions reproduce the response. |
...
| BUG-017 | P2       | After the type-size correction, placeholder, property-label, and survey-disclaimer colors still measured only 3.62–3.77:1 against the limestone panel.                                                                                                              | Microcopy colors were chosen as quiet visual accents without enforcing the normal-text contrast floor.                                 | Those roles now use the existing slate token, measured at 4.97:1 against the solid panel; production-browser assertions lock the computed color.                                                                                                      |
| BUG-018 | P1       | The final aggregate `test:coverage` gate reported 99.81% branch coverage because the new result summary had not exercised pending “Searching Nashville” or singular “1 match” copy.                                                                                 | The UI state matrix gained two display branches without matching component-state cases.                                                | Explicit pending and singular render states restore 100% branch coverage without exclusions; the complete coverage and production-browser campaigns were rerun.                                                                                       |

## Shared-cause review

Twelve shared causes account for the findings:
...
10. Visual density was optimized before a minimum persistent-information type
    scale was enforced.
11. Muted microcopy colors were not checked against the normal-text contrast
    threshold.
12. New conditional interface copy was not added to the component-state test
...
The same persistent-text boundary now enforces a slate contrast floor on solid
panels.
The release gate now covers every pending, singular, plural, empty, and closed
...
- The first aggregate `npm run test:coverage` run retained BUG-018 evidence at
  99.81% branches; the focused rerun then passed 25 files and 150 tests at 100%
  statements (2,254/2,254), branches (1,080/1,080), functions (472/472), and
  lines (2,048/2,048).
- Final `npm run test:e2e`: production client and SSR builds passed, then 25 of
  25 browser journeys passed in one run: 10 risk-inventory journeys, 8
  interaction/provider/performance journeys, and 7 visual baselines.
...
- The campaign did not read the local Google API key; Google behavior used
  controlled browser fixtures. One public Metro geocoder query for
  `600 CHURCH ST` verified that the real six-result response now renders as two
...
- Vite reports one route chunk over its generic 500 kB minified warning
  threshold. It is 162.65 kB gzip and primarily contains the single-route 3D map
